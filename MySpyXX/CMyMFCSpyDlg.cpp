// CMyMFCSpyDlg.cpp : implementation file
//
...
static std::map<int, const char*> g_wmTranslation = {
{0, "WM_NULL" },
{1, "WM_CREATE" },
{2, "WM_DESTROY" },
{3, "WM_MOVE" },
...
IMPLEMENT_DYNAMIC(CMyMFCSpyDlg, CDialogEx)

CMyMFCSpyDlg::CMyMFCSpyDlg(HWND hWnd,CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_CMyMFCSpyDlg, pParent),
	m_hCurSelWnd{ hWnd }
{

}

CMyMFCSpyDlg::~CMyMFCSpyDlg()
{
	if (m_hDll)
	{
		::FreeLibrary(m_hDll);
...
void CMyMFCSpyDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT_MFCSPYDLG_INFO, m_edtInfo);
}


BEGIN_MESSAGE_MAP(CMyMFCSpyDlg, CDialogEx)
	ON_MESSAGE(WM_APP, &CMyMFCSpyDlg::OnApp)
END_MESSAGE_MAP()


// CMyMFCSpyDlg message handlers


BOOL CMyMFCSpyDlg::OnInitDialog()
{
	...
	m_hDll = ::LoadLibrary(TEXT("MFCSpyhk"));
	
	using PFN_SetSharedData = void(*)(DWORD dwCallerThreadId, HHOOK hHook, HWND  hTargetWnd);
	auto pfnSetSharedData = (PFN_SetSharedData)::GetProcAddress(m_hDll, "SetSharedData");

	auto hookFunc = (HOOKPROC)::GetProcAddress(m_hDll, "MFCSpyHookFunc");

	DWORD processId;
	auto tid = GetWindowThreadProcessId(m_hCurSelWnd, &processId);

	m_hHook = ::SetWindowsHookEx(WH_GETMESSAGE, hookFunc, m_hDll, tid);
	pfnSetSharedData(::GetCurrentThreadId(), m_hHook, m_hCurSelWnd);
	::PostThreadMessage(tid, WM_NULL, 0, 0);	//force to load dll
...
using PFN_GETSPYINFO = tagSpyInfo * (*)();

afx_msg LRESULT CMyMFCSpyDlg::OnApp(WPARAM wParam, LPARAM lParam)
{
	CString strInfo;

	strInfo = TEXT("Class Info:\r\n");
	
	auto pfnGetSpyInfo = (PFN_GETSPYINFO)::GetProcAddress(m_hDll, "SpyGetInfo");

	auto spyInfo = pfnGetSpyInfo();


	for (size_t i = 0; i < spyInfo->g_nNumOfClasses; i++)
	{
		USES_CONVERSION;
		strInfo = strInfo  + A2T(spyInfo->g_szAllClassName[i]) + TEXT("\r\n");
	}


	strInfo = strInfo + TEXT("\r\nMessageMap Table\r\n") + TEXT("strMessage\t\tnMessage	\tnCode	\tnID	\tnLastID	\tpfn\r\n");
	for (size_t i = 0; i < spyInfo->g_nMsgEntry; i++)
	{
		USES_CONVERSION;
		strInfo.Format(TEXT("%s%-20s\t\t%08X\t%08X\t%08X\t%08X\t%p\r\n"), strInfo,
			A2T(g_wmTranslation[spyInfo->g_MsgMap[i].nMessage]),
			spyInfo->g_MsgMap[i].nMessage,
			spyInfo->g_MsgMap[i].nCode, 
			spyInfo->g_MsgMap[i].nID, 
			spyInfo->g_MsgMap[i].nLastID, 
			spyInfo->g_MsgMap[i].pfn);
	}

	std::vector<CString> virtualFunc = {
		TEXT("CObject GetRuntimeClass"),
		TEXT("CObject ~Dtor"),
		TEXT("CObject Serialize"),
		
		TEXT("CCmdTarget OnCmdMsg"),
		TEXT("CCmdTarget OnFinalRelease"),
		TEXT("CCmdTarget IsInvokeAllowed"),
		TEXT("CCmdTarget GetDispatchIID"),
		TEXT("CCmdTarget GetTypeInfoCount"),
		TEXT("CCmdTarget GetTypeLibCache"),
		TEXT("CCmdTarget GetTypeLib"),
		TEXT("CCmdTarget GetMessageMap"),
		TEXT("CCmdTarget GetCommandMap"),
		TEXT("CCmdTarget GetDispatchMap"),
		TEXT("CCmdTarget GetConnectionMap"),
		TEXT("CCmdTarget GetInterfaceMap"),
		TEXT("CCmdTarget GetEventSinkMap"),
		TEXT("CCmdTarget OnCreateAggregates"),
		TEXT("CCmdTarget GetInterfaceHook"),
		TEXT("CCmdTarget GetExtraConnectionPoints"),
		TEXT("CCmdTarget GetConnectionHook"),

		TEXT("CWnd PreSubclassWindow"),
		TEXT("CWnd Create"),
		TEXT("CWnd CreateEx"),
		TEXT("CWnd DestroyWindow"),
		TEXT("CWnd PreCreateWindow"),
		TEXT("CWnd CalcWindowRect"),

		//....TODO too many
	};


	std::vector<CString> virtualFuncDebugOrDll = {
		TEXT("CObject GetRuntimeClass"),
		TEXT("CObject ~Dtor"),
		TEXT("CObject Serialize"),

		TEXT("CObject AssertValid"),
		TEXT("CObject Dump"),
	...
	strInfo = strInfo + TEXT("\r\nVirtual Func\r\n");
	auto pStrVecVirtualFunc = &virtualFunc;
		if (spyInfo->g_bIsDebug || spyInfo->g_bIsWndProcInDll)
		{
			pStrVecVirtualFunc = &virtualFuncDebugOrDll;
		}

		for (size_t i = 0; i < pStrVecVirtualFunc->size(); i++)
		{
			strInfo.Format(TEXT("%s%s->0x%p\r\n"),strInfo, (*pStrVecVirtualFunc)[i],spyInfo->vTable[i]);
		}
